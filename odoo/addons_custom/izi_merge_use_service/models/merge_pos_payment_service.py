...
class MergePosPaymentService(models.Model):
    _name = 'merge.pos.payment.service'

    def _default_amount(self):
        active_id = self.env.context.get('active_id')
        if active_id:
            order = self.env['merge.use.service'].browse(active_id)
            tmp = 0
            for line in order.use_service_ids:
                tmp += line.amount
            tmp1 = 0
    ...
    def _compute_partner_id(self):
        active_id = self.env.context.get('active_id')
        if active_id:
            self.x_partner_id = self.env['merge.use.service'].browse(active_id).customer_id
        else:
    ...
    merge_using_service_id = fields.Many2one('merge.use.service', ' Merge Using service')
    ...
    @api.onchange('merge_using_service_id')
    def _on_change_using_service(self):
        if self.merge_using_service_id:
            pos_session = self.env['pos.session']
            pos_config_id = self.env.user.x_pos_config_id.id
            my_session = pos_session.search([('config_id', '=', pos_config_id), ('state', '=', 'opened')])
            return {
    ...
    def onchange_journal_id(self):
        registed = 0.0
        for line in self.merge_using_service_id.pos_payment_service_ids:
            registed += line.amount
        registed -= self.amount
        self.amount = self.merge_using_service_id.amount_total - registed
        ...
        if self.journal_id:
            ...
            elif self.journal_id.id == self.merge_using_service_id.pos_session_id.config_id.journal_deposit_id.id:
    ...
    def process_payment_service(self):
        ...
        if self._default_amount() == 0:
            # self.merge_using_service_id.update({'state': 'wait_approve'})
            return {'type': 'ir.actions.act_window_close'}
        else:
    ...
    def launch_payment(self):
        ...
        ctx.update({'default_merge_using_service_id': self.merge_using_service_id.id})
