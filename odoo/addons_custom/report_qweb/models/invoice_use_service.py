...
class UseServiceQweb(models.Model):
    ...
    def action_print(self):
        if self.pos_session_id.branch_id.brand_id.code in ['KOREA']:#Gangnam
            return {
    ...
    @api.multi
    def action_print_department_rate(self):
        if self.pos_session_id.branch_id.brand_id.code in ['KOREA']:#Gangnam
            return {
                'type': 'ir.actions.act_url',
                'url': 'report/pdf/report_qweb.rate_department_report/%s' %(self.id),
                'target': 'new',
                'res_id': self.id,
            }
        else:
    ...
    def get_body_history(self):
        body_measures = self.env['therapy.body.measure'].search([('therapy_record_id', '=', self.therapy_record_id.id)], order ='measurement_time DESC', limit=5)
