...
class ListBookingByEmployee(models.TransientModel):
    ...
    def general_booking(self):
        ...
        sql = '''SELECT a.name booking, c.id employee_id, a.team_id, a.time_from, a.time_to, a."time",A.customer_id, A.contact_number 
        ...
        if len(lists) >= 1:
            for i in lists:
                obj_line.create({
                    ...
                    'partner_id': i['customer_id'] and int(i['customer_id']) or False,
                    'contact_number': i['contact_number'] and i['contact_number'] or False,
        ...
        action = {
            ...
            'views': [(self.env.ref('izi_crm_booking.list_booking_by_employee_line_view_timeline').id, 'timeline')],
            # 'context':{
