...
class ReceptionCustomer(models.Model):
    ...
    def action_search_lead(self):
        ...
        leads = Obj_lead.search(['|', ('partner_name', 'ilike', self.name), ('phone', '=', self.name), ('x_state', 'in', ['lead', 'reference', 'reference2', 'opportunity', 'meeting']), ('team_id', 'in', team_ids)])
    ...
    def action_assign_lead(self):
        if self.search_result == 'not_search' or self.search_result == 'not_found':
            ...
            stage_id = self.env['crm.stage'].search([('x_code', '=', 'to_shop')])
            if not stage_id:
