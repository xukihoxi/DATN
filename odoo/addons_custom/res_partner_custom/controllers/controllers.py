        ...
        if model == 'res.partner':
            ...
            if not (uid == 1):
                if group_revenue_control:
                    domain = domain
                elif group_cost_accountant or group_customer_care or group_leader_customer_care or group_revenue_accountant or group_business_manager or group_receptionist or group_cashier or group_leader_shop or group_inventory_accounting:
                    phone = ''
                    for item in domain:
                        if phone: break
                        ...
                        if type(item) is list:
                            for attr in item:
                                if attr == 'phone':
                                    ...
                                    break
                    if phone:
                        partner = ObjPartner.search(
                            ['|', ('phone', '=', phone), ('mobile', '=', phone), ('x_brand_id', 'in', brand_ids)],
                            limit=1)
                        if partner:
                            domain = [('id', '=', partner.id)]
                        else:
