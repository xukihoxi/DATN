...
class IziCrmLead(models.Model):
    ...
    x_state = fields.Char(related='stage_id.x_code', string='State', store=True)
    ...
    def _onchange_name(self):
        if not self.partner_id and self.x_config_id:
    ...
    def action_pos(self):
        ...
        # self.x_state = 'won'
        stage_id = self.env['crm.stage'].search([('x_code', '=', 'win')], limit=1)
        self.stage_id = stage_id.id
        if len(self.x_lines) != 0:
