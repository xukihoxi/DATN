...
class Claim(models.Model):
    ...
    name = fields.Char(related='partner_id.name', string="Subject")
    ...
    type_ids = fields.Many2many('crm.claim.type', string="Type", track_visibility='onchange')
    ...
    follower_ids = fields.Many2many('res.users', string="Followers", track_visibility='onchange')
    ...
    def action_assign(self):
        if self.handler_id and self.deadline:
            if self.state != 'new': raise except_orm("Thông báo", "Phản hồi đã thay đổi trạng thái vui lòng tải lại trang web!")
    ...
    def action_handle(self):
        if self.cause and self.resolution:
            if self.state != 'processing': raise except_orm("Thông báo", "Phản hồi đã thay đổi trạng thái vui lòng tải lại trang web!")
            self.write({
                'state': "done"
            })
        else:
