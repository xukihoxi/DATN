...
class InvoiceMakePayment(models.TransientModel):
    ...
    def _allocation_revenua(self, amount_total, order_id, session_id):
        ...
        oder_id = self.env['pos.order'].search([('id', '=', order_id.id)])
        oder_id.x_total_order += amount_total
        for x in revenua_old:
            for y in x.line_ids:
                if y.amount >- 1000 and y.amount < 1000 or y.amount == 0:
                    y.unlink()
        ...
        if revenue_id.amount_res > -1000 and revenue_id.amount_res < 1000:
            revenue_id.amount_res = 0
