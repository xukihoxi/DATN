...
class PosUserMaterialTmp(models.Model):
    _name = 'merge.tmp.service.card.using'

    # name = fields.Char("Nam")
    user_service_card_id = fields.Many2one('merge.use.service', string="Service Card Using")
    lines = fields.One2many('merge.tmp.service.card.using.line','tmp_service_card_using', string="Detail Using Material")
...
class PosUserMaterialLineTmp(models.Model):
    _name = 'merge.tmp.service.card.using.line'

    tmp_service_card_using = fields.Many2one('merge.tmp.service.card.using', string = 'Tmp user card ')
