...
class PosDestroyService(models.Model):
    ...
    def get_x_is_sign_order(self):
        for i in self:
            if i.session_id.config_id.x_is_sign_order == True:
    ...
    def process_destroy_service(self):
        if self.signature_image == False and self.x_is_sign_order == True:
    ...
    def action_done(self):
        if self.session_id.config_id.x_is_sign_order == False:
            self.process_rate_service()
        if self.state == 'done':
