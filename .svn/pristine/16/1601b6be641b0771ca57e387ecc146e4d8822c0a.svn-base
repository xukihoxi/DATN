...
class ResUsers(models.Model):
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        ...
        if 'domain_by_telesale' in self._context:
            ids = []
            job_ids = self.env['hr.job'].search([('x_code', 'in', ['TELESALE', 'TVV'])])
            employee_ids = self.env['hr.employee'].search([('job_id.id', 'in', job_ids.ids)])
            for emp in employee_ids:
                if emp.user_id:
                    ids.append(emp.user_id.id)
            print(ids)
            res = self.search([('id', 'in', ids)], limit=limit)
        return res.name_get()
